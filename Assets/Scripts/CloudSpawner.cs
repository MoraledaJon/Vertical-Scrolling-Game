...
public class CloudSpawner : MonoBehaviour
{
	...
	public float spawnIntervalBlack = 6.0f; // Time between each spawn
    ...
    private void Start()
    {
		...
		StartCoroutine(SpawnBlackClouds());
	}
    ...
    private IEnumerator SpawnClouds()
    {
        while (true)
        {
			if (!isPaused) // Check if not paused
            {
				...
				GameObject tempCloud = Instantiate(cloudPrefab[0], Vector3.zero, Quaternion.identity);
				...
				GameObject cloud = Instantiate(cloudPrefab[0], spawnPosition, Quaternion.identity);
	...
	private IEnumerator SpawnBlackClouds()
	{
		yield return new WaitForSeconds(spawnIntervalBlack /2);
		while (true)
		{
			if (!isPaused) // Check if not paused
			{
				...
				// Calculate additional offset to ensure the cloud is off-screen
				GameObject tempCloud = Instantiate(cloudPrefab[1], Vector3.zero, Quaternion.identity);
				tempCloud.transform.localScale = new Vector3(scale, scale, 1);
				float offset = tempCloud.GetComponent<SpriteRenderer>().bounds.extents.x;
				Destroy(tempCloud);
				...
				GameObject cloud = Instantiate(cloudPrefab[1], spawnPosition, Quaternion.identity);
				cloud.transform.localScale = new Vector3(scale, scale, 1);
			...
			yield return new WaitForSeconds(Random.Range(spawnIntervalBlack, spawnIntervalBlack * 2f));
		}
	}
